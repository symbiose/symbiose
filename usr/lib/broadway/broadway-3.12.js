...
// ADDS FOR SYMBIOSE BELOW

log = function (str) {
    console.log('broadway: '+str);
}

start = function ()
{
    setupDocument(options.document || document);
...
connect = function (opts)
{
    ...
    if (opts.handleCommands) {
        //handleCommands = opts.handleCommands;
    }
    if (opts.cmd) {
        if (opts.cmd.createSurface) {
            cmdCreateSurface = function (id, x, y, width, height, isTemp) {
                var surface = opts.cmd.createSurface.apply(null, arguments);

                surfaces[id] = surface;
            };
        }
        if (opts.cmd.showSurface) {
            cmdShowSurface = opts.cmd.showSurface;
        }
        if (opts.cmd.hideSurface) {
            cmdHideSurface = opts.cmd.hideSurface;
        }
        if (opts.cmd.setTransientFor) {
            cmdSetTransientFor = opts.cmd.setTransientFor;
        }
        if (opts.cmd.deleteSurface) {
            cmdDeleteSurface = opts.cmd.deleteSurface;
        }
        if (opts.cmd.moveResizeSurface) {
            cmdMoveResizeSurface = opts.cmd.moveResizeSurface;
        }
        if (opts.cmd.flushSurface) {
            cmdFlushSurface = opts.cmd.flushSurface;
        }
        restackWindows = function () {};
    }
