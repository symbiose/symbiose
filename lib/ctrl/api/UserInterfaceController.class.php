...
class UserInterfaceController extends \lib\ApiBackController {
	protected function _detectMobileBrowser() {
	...
	public function executeLoadBooter($uiName = null) {
		...
		$uiLabels = array(($user->isLogged()) ? 'userInterface' : 'guestInterface');

		if ($this->_detectMobileBrowser()) {
			$uiLabels[] = 'touchDevices';
		}

		if (empty($uiName)) {
			$ui = $manager->getByLabelsList($uiLabels);
			if (empty($ui)) {
